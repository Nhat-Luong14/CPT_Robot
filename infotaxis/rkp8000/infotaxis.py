...
def build_log_src_prior(prior_type, xs, ys):
    ...
    mask = get_mask(log_src_prior.shape)
...
def get_p_src_found(pos, xs, ys, log_p_src, radius):
    ...
    mask = (dxs**2 + dys**2 < radius**2)                        #cell has distance inside the radius is set true
...
def get_p_sample(pos, xs, ys, dt, h, w, d, r, a, tau, log_p_src):
    ...
    hit_rate = get_hit_rate(xs, ys, pos, w, d, r, a, tau)       #matrix form (hit/ miss rate at a position given a matrix of possible src)
...
def update_log_p_src(pos, xs, ys, dt, h, w, d, r, a, tau, src_radius, log_p_src):
    ...
    mean_hits = dt * get_hit_rate(xs_src=xs, ys_src=ys, pos=pos, w=w, d=d, r=r, a=a, tau=tau)
...
def simulate(plume, grid, start_pos, speed, dt, max_dur, th, src_radius, w, d, r, a=0.003, tau=100):
    ...
    hs      = []            # hit sequence
    ss      = []            # entropy sequence
    mode    = []            # path generated by Djkstra
    path    = []    
    ...
    for t_ctr, t in enumerate(np.arange(0, max_dur, dt)):
        ...
        # log the changing of information
        s = entropy(log_p_src)
        ss.append(s)
        ...
        for move in moves:
            ...
            p_src_found = get_p_src_found(pos=move, xs=xs, ys=ys, log_p_src=log_p_src, radius=src_radius)
            ...
            for ctr, h in enumerate(sample_domain):
                ...
                p_sample = get_p_sample(pos=move, xs=xs, ys=ys, dt=dt, h=h,
