...
def test_get_p_sample_works_qualitatively_for_examples():
    ...
    log_p_src[~mask] = -np.inf         #outside the source region, all other place has minus infinity of log probabily or 0% of source is there
...
#ok
def show_hit_rate_map_for_example_parameters(pos=(1.5, 0.7), w=0.5, d_low=0.02, d_high=0.2, r=100, a=0.003, tau=1000):
    """
    ...
    for d, label, ax_col in zip([d_low, d_high], ['low', 'high'], axs.T):
        hit_rate = get_hit_rate(xs_src=xs_src, ys_src=ys_src, pos=pos, w=w, d=d, r=r, a=a, tau=tau)

        # hit rate map
        ax_col[0].imshow(hit_rate.T, origin='lower', extent=extent, zorder=0, cmap='hot')
        # sample position
        ax_col[0].scatter(pos[0], pos[1], color='g', marker='D', s=100, lw=0, zorder=1)
        # wind direction
        ax_col[0].arrow(.5, .2, 1, 0, head_width=.05, head_length=.05, lw=4, fc='w', ec='w')
        ...
        for ctr, color in enumerate((.2, .4, .6, .8)):
            ...
            handles.append(ax_col[1].plot(xs_src, hit_rate_, lw=3, color=color_, label=label)[0])
...
"""
Plot the resulting source posteriors (where prior is uniform) 
following miss and hit at given sampling position.
"""
def show_update_log_p_src_gives_correct_qualitative_behavior_for_examples(
    pos=(1.5, 0.7), dt=0.1, w=0.5, d=0.05, r=100, a=0.003, tau=1000, src_radius=0.02):
    ...
    log_p_src_miss = update_log_p_src(pos=pos, xs=xs, ys=ys, dt=dt, h=0, w=w,d=d, r=r, a=a, tau=tau, src_radius=src_radius, log_p_src=log_src_prior)
...
def show_infotaxis_demo(
        seed=8, grid=(101, 51), src_pos=(.1, .5), start_pos=(1.9, .9),
    ...
    plume = IdealInfotaxisPlume(src_pos=src_pos, w=w, d=d, r=r, a=a, tau=tau, dt=dt)
